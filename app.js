...
const I18N = {
  en: {
    ...
    startingXI: "Starting XI", designedBy: "Designed by", pens: "Pens", wonPens: "Won on penalties",
  ...
  pt: {
    ...
    startingXI: "Onze inicial", designedBy: "Design por", pens: "G.P.", wonPens: "Venceu nas grandes penalidades",
...
function teamCell(m, side) {
  const tm = side === "home" ? m.HomeTeam : m.AwayTeam;
  const won = m._pens && m._pens.winner === side;
  return `<div class="team ${won ? "won" : ""}" ${FLAG[tm] ? `data-squad="${tm}"` : ""}>
      <span class="t-flag">${flagHtml(tm)}${won ? `<span class="adv-tick" title="${t("wonPens")}">✓</span>` : ""}</span>
      <span class="t-name">${teamLabel(tm)}</span>
    </div>`;
}
...
function matchCard({ m, p }, i) {
  ...
  const ftLabel = m._pens ? `${t("pens")} ${esc(m._pens.home)}–${esc(m._pens.away)}` : t("fullTime");
  const mid = live
    ? `<span class="mc-score">${hasScore ? score : "·"}</span><span class="mc-status live">● ${esc(clock || t("live"))}</span>`
    : finished
      ? `<span class="mc-score">${hasScore ? score : "—"}</span><span class="mc-status ft">${ftLabel}</span>`
...
async function openMatch(num) {
  ...
  const mid = live
    ...
    : finished
      ? `<span class="ms-score">${hasScore ? score : "—"}</span><span class="ms-state ft">${ftLabel}</span>`
      : `<span class="ms-kick">${p.time}</span><span class="ms-state">${p.dow} ${p.dom} ${p.mon}</span>`;

  const side = (team, which) => {
    const won = m._pens && m._pens.winner === which;
    return `<div class="ms-side ${won ? "won" : ""}" ${FLAG[team] ? `data-squad="${team}"` : ""}>
      <span class="t-flag">${flagHtml(team, "ms-flag")}${won ? `<span class="adv-tick" title="${t("wonPens")}">✓</span>` : ""}</span>
  ...
  $("#matchInner").innerHTML = `
    <div class="squad-hero ms-hero">
      ...
      <div class="ms-fixture">
        ${side(m.HomeTeam, "home")}
        <div class="ms-mid">${mid}</div>
        ${side(m.AwayTeam, "away")}
      </div>
...
async function refreshLive() {
  ...
  for (const date of dates) {
    ...
    for (const ev of (data && data.events) || []) {
      ...
      const before = JSON.stringify([m.HomeTeamScore, m.AwayTeamScore, m._st, m._pens, MATCH_GOALS[m.MatchNumber]]);
      ...
      // penalty shootout — keep who advanced even when the score is level
      if (H.shootoutScore != null && H.shootoutScore !== "" && A.shootoutScore != null && A.shootoutScore !== "") {
        m._pens = { home: String(H.shootoutScore), away: String(A.shootoutScore), winner: H.winner ? "home" : "away" };
        if (m._pens.winner === "home") m.Winner = m.HomeTeam; else m.Winner = m.AwayTeam;
      }
...
(function splash() {
  ...
  try { sessionStorage.setItem("wcc-splash", "1"); } catch { /* private mode */ }
  let done = false;
  const dismiss = () => { if (done) return; done = true; el.classList.add("out"); setTimeout(() => el.remove(), 650); };
  const timer = setTimeout(dismiss, 2400);
  el.addEventListener("click", (e) => { if (e.target.closest("a")) return; clearTimeout(timer); dismiss(); });
})();
